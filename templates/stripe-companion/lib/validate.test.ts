...
test("valid action + valid slug passes", () => {
  const r = validateBeforeFire("mrr-snapshot.read", { slug: SLUG });
  expect(r.ok).toBe(true);
});

test("unknown action is rejected", () => {
  // Cast through unknown: the closed union would reject this at compile time,
...
test("malformed slug is rejected before any network call", () => {
  const r = validateBeforeFire("mrr-snapshot.read", { slug: BAD_SLUG });
  expect(r.ok).toBe(false);
  if (!r.ok) expect(r.reason).toContain("slug shape invalid");
});

test("duplicate in-flight request is rejected", () => {
  const release = markInFlight("mrr-snapshot.read", SLUG);
  try {
    const r = validateBeforeFire("mrr-snapshot.read", { slug: SLUG });
    expect(r.ok).toBe(false);
...
test("a different action+slug is independent of another's in-flight slot", () => {
  const release = markInFlight("mrr-snapshot.read", SLUG);
  try {
    // failed-payments.read on the same slug has its own bucket; not blocked
    // by the mrr-snapshot in-flight slot, and no prior fire so no rate floor.
    const r = validateBeforeFire("failed-payments.read", { slug: SLUG });
    expect(r.ok).toBe(true);
  } finally {
    release();
  }
