/**
 * stripe-proxy HTTP client with SelrAI-HMAC v1 signing.
 *
 * HMAC key derivation MUST match the Worker's computation
 * (`stripe-proxy/src/durable-objects/tenant.ts computeHmac`):
 *
 ...
 *
 * The double conversion (bytes -> hex -> bytes) is intentional and matches
 * the Worker's stored shape. Do not skip the hex round-trip; the signatures
 * will mismatch.
 *
 * If you are porting this to a new client (Telegram bot, voice bridge, n8n
 * caller, CLI), port deriveKey byte-for-byte and verify against
 * stripe-proxy/tests/hmac-smoke.sh with a known seed before doing anything
 * else.
 */
...
export interface MrrSnapshotResponse {
  slug: string;
  currency: string;
  mrr_cents: number;
  mom_delta_cents: number;
...
async function signedFetch(creds: Credentials, path: string): Promise<Response> {
...
async function signedAction<T>(action: ActionName, creds: Credentials): Promise<T> {
...
export async function fetchMrrSnapshot(creds: Credentials): Promise<MrrSnapshotResponse> {
  return signedAction<MrrSnapshotResponse>("mrr-snapshot.read", creds);
}

export async function fetchFailedPayments(creds: Credentials): Promise<FailedPaymentsResponse> {
  return signedAction<FailedPaymentsResponse>("failed-payments.read", creds);
}

/**
 * Validate a `stripeproxy://pair?url=...&slug=...&secret=...` URI and return
 * the parsed credentials. Returns null on any validation failure. Mirror of
 * xero-companion's parsePairUri; only difference is the URI scheme prefix.
 */
export function parsePairUri(uri: string): Credentials | null {
  if (!uri.startsWith("stripeproxy://pair")) return null;
