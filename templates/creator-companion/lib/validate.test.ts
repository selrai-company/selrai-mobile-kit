...
test("valid action + valid slug passes", () => {
  const r = validateBeforeFire("daily-prompt.read", { slug: SLUG });
...
test("malformed slug is rejected before any network call", () => {
  const r = validateBeforeFire("daily-prompt.read", { slug: BAD_SLUG });
  expect(r.ok).toBe(false);
  if (!r.ok) expect(r.reason).toContain("slug shape invalid");
});

test("duplicate in-flight request is rejected", () => {
  const release = markInFlight("daily-prompt.read", SLUG);
  try {
    const r = validateBeforeFire("daily-prompt.read", { slug: SLUG });
    expect(r.ok).toBe(false);
...
test("a different action+slug is independent of another's in-flight slot", () => {
  const release = markInFlight("daily-prompt.read", SLUG);
  try {
    // post-draft.read on the same slug has its own bucket; not blocked by the
    // daily-prompt in-flight slot, and no prior fire so no rate floor.
    const r = validateBeforeFire("post-draft.read", { slug: SLUG });
    expect(r.ok).toBe(true);
  } finally {
    release();
  }
