/**
 * ghl-proxy HTTP client with SelrAI-HMAC v1 signing.
 *
 * HMAC key derivation MUST match the Worker's computation
 * (`ghl-proxy/src/durable-objects/tenant.ts computeHmac`):
 *
 ...
 * ghl-proxy/tests/hmac-smoke.sh with a known seed before doing anything
 * else.
 */
...
export interface DailyPromptResponse {
  ...
  date: string;
  prompt: string;
  theme: "value" | "story" | "behind-the-scenes" | "cta";
...
export async function fetchDailyPrompt(creds: Credentials): Promise<DailyPromptResponse> {
  return signedAction<DailyPromptResponse>("daily-prompt.read", creds);
}

export async function fetchPostDraft(creds: Credentials): Promise<PostDraftResponse> {
  return signedAction<PostDraftResponse>("post-draft.read", creds);
}

/**
 * Validate a `ghlproxy://pair?url=...&slug=...&secret=...` URI and return
...
export function parsePairUri(uri: string): Credentials | null {
  if (!uri.startsWith("ghlproxy://pair")) return null;
