 ...
 * source of truth for "what reads we are allowed to fire".
 *
 ...
 * markInFlight(name, slug) in lib/xero-client.ts. The validate gate refuses
 * any call to an unknown action name.
 */

export type ActionName = "cash-flow.read" | "ar-ageing.read";
...
export interface ActionDef {
   ...
   * burning two Worker requests + two Xero API calls. */
...
export const ACTION_REGISTRY: Record<ActionName, ActionDef> = {
  "cash-flow.read": {
    method: "GET",
    pathTemplate: "/:slug/cash-flow",
    minIntervalMs: 3000,
  },
  "ar-ageing.read": {
    method: "GET",
    pathTemplate: "/:slug/ar-ageing",
    minIntervalMs: 3000,
  },
};
