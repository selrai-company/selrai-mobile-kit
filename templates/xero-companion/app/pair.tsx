...
import { parsePairUri } from "../lib/xero-client";
...
export default function PairScreen() {
  ...
  const onPair = useCallback(
    async (uri: string) => {
      ...
      if (!creds) {
        setError("Invalid pair URI. Check the QR code or paste the full xeroproxy:// link.");
        return;
      }
  ...
  return (
    <>
      <Stack.Screen options={{ title: "Pair Xero Companion" }} />
      <View className="flex-1 bg-slate-50 px-5 pt-5">
        <Text className="text-2xl font-bold text-slate-800 mb-2">
          Pair this app to your Xero proxy
        </Text>
        ...
        <View className="flex-row mb-4 rounded-lg overflow-hidden border border-teal-700">
          <Pressable
            onPress={() => setMode("camera")}
            className={`flex-1 py-2 items-center ${mode === "camera" ? "bg-teal-700" : "bg-white"}`}
            accessibilityRole="button"
            accessibilityLabel="Scan QR code"
          >
            <Text className={`font-medium ${mode === "camera" ? "text-white" : "text-teal-700"}`}>
              Scan QR
            </Text>
          </Pressable>
          <Pressable
            onPress={() => setMode("paste")}
            className={`flex-1 py-2 items-center ${mode === "paste" ? "bg-teal-700" : "bg-white"}`}
            accessibilityRole="button"
            accessibilityLabel="Paste pair URI instead"
          >
            <Text className={`font-medium ${mode === "paste" ? "text-white" : "text-teal-700"}`}>
              Paste URI
            </Text>
          </Pressable>
        </View>
        ...
        {mode === "camera" ? (
          <View className="h-80 rounded-2xl overflow-hidden border border-teal-700">
            {!permission ? (
          ...
          <View>
            <Text className="text-sm text-slate-700 mb-2">
              Paste the full pair URI (begins with xeroproxy://pair?...)
            </Text>
            <TextInput
              ...
              autoCorrect={false}
              multiline={false}
              placeholder="xeroproxy://pair?url=...&amp;slug=...&amp;secret=..."
              accessibilityLabel="Pair URI input"
            />
            <Pressable
              onPress={onPasteSubmit}
              className="rounded-2xl bg-teal-700 py-3 items-center"
              accessibilityRole="button"
